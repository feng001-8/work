...
/**
 * @title InfinityDaoLaunchPad
 * @dev 启动板合约，实现代币发行和预售功能
 * @notice 该合约支持双阶段发行模式：预售阶段和公开发行阶段
 */
contract InfinityDaoLaunchPad {
    ...
    /**
     * @dev 内部函数，从指定地址转账代币到合约
     * @param _token 代币合约接口
     * @param _ammount 转账数量
     * @param _from 转账来源地址
     * @return 转账是否成功
     * @notice 用于从用户地址转账代币到合约地址
     */
